...
class lstm(torch.nn.Module):
    ...
    def forward(self,inputs,device=None):
        ...
        for X in embeddings:
            ...
            I = self.sig(IFOC[:,:self.num_hiddens])
            F = self.sig(IFOC[:,self.num_hiddens:2*self.num_hiddens])
            O = self.sig(IFOC[:,2*self.num_hiddens:3*self.num_hiddens])
            C_tilda = self.tan(IFOC[:,:self.num_hiddens])
        ...
        temp_1 = torch.cat(outputs,dim=0)
